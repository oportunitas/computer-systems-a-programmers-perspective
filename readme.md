...
...in a sense that you cant map every permutation of any number of binary digits to any number of decimal digits *(10 is not a power of 2).* but representing an entire byte with a base-256 notation would be the most compact but would be way too unintuitive (at least imo, what symbols would you even need, there's so many keys in a keyboard), so representing 8 bits via a pair of 4 bit-representation through a single hexadecimal digit sounds like the perfect middle ground, and probably is the actual reason behind the convention.
